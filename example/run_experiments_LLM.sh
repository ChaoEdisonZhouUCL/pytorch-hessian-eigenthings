...
if [[ "$HOSTNAME" == *"juwels"* ]]; then
    ...
    $ENV_ACTIVATION python -m pip install openpyxl
...
echo "==============================================="
echo "Running experiment: SEED=$seed, LR=$lr, OPT=$optimiser, step=$step"
echo "==============================================="

# Create experiment-specific directory
EXP_DIR="$RESULTS_DIR/${optimiser}_seed_${seed}_lr_${lr}"
mkdir -p "$EXP_DIR"

# Log file for this experiment
LOG_FILE="$EXP_DIR/experiment.log"
echo "Starting experiment: SEED=$seed, OPT=$optimiser, LR=$lr" > "$LOG_FILE"
...
CHECKPOINT_PATTERN="/home/c01chzh/CISPA-projects/pt_network-2024/NanoAdam/huggingface_glue_mnli/results/Tom_${dataset}_bert-base_${optimiser}_lr${lr}_seed${seed}_*/checkpoint-${step}"
CHECKPOINT_DIR=$(find . -type d -path "$CHECKPOINT_PATTERN" -print -quit)

if [ -z "$CHECKPOINT_DIR" ]; then
  echo "ERROR: No checkpoint found for opt=$optimiser, seed=$seed, lr=$lr" | tee -a "$LOG_FILE"
  echo "ERROR: No checkpoint found for opt=$optimiser, seed=$seed, lr=$lr" >> "$SUMMARY_FILE"
else
...
EIGENVAL_OUTPUT="$EXP_DIR/eigenvalues_step${step}.txt"

$ENV_ACTIVATION $TORCHRUN_CMD example/main_LLM.py \
    --mode="$MODE" \
    --dataset="$dataset" \
    ...
    --output_excel="$EXP_DIR/eigenvalues_seed${seed}_lr${lr}_step${step}.xlsx" \
